...
> {
  ...
  static allocateStorage<
    ...
    BK extends string = any,
    OK extends string = any
  >(namespace: string, keys: KeysOptions<SK, NK, BK, OK> = {}) {
    ...
    others = others || ([] as OK[]);
    const storage: AnyRecord = new Storage<SK, NK, BK, OK>(
      namespace,
      strings,
      numbers,
      booleans,
    ...
    (<Array<SK | NK | BK | OK>>[])
      .concat(strings, numbers, booleans, others)
      .forEach((key) => (storage[key] = storage.readItem(key)));
  ...
  read = this.readItem.bind(this);
  write = this.writeItem.bind(this);
