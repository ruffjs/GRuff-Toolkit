...
class ListRDO<T extends any = any> extends AbstractRDO<T[]> {
  ...
  constructor(model: any, min: number, max: number) {
    ...
    let r0 = parseInt(min as unknown as string) || 1,
      r1;
    r0 = r0 >= 0 ? r0 : 0;
  ...
  [generate](random: any = {}, context: object = {}): T {
    ...
    this.inputs.forEach((rdo, i) => {
      ...
      string = string.replaceAll(
        `{${i}}`,
        String(rdo[generate](random, context))
      );
    });
    return string as T;
  }
...
function createRDOByObject(exp: {
  ...
  switch (exp.type) {
    ...
    case "list":
      if (exp.model) {
        ...
        if (typeof exp.count === "number") {
          ...
          typeof exp.count === "object" &&
          exp.count instanceof Array &&
          typeof exp.count[0] === "number"
        ) {
    ...
    case "call":
      if (exp.method) {
        const args =
          typeof exp.args === "object" && exp.args instanceof Array
            ? exp.args
            : [];
    ...
    case "str":
      if (typeof exp.template === "string") {
        if (exp.template !== "") {
          const inputs =
            typeof exp.inputs === "object" && exp.inputs instanceof Array
              ? exp.inputs
              : [];
