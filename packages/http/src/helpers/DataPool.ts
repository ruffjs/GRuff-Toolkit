...
export type MappingLike<T> = T & { getTarget: () => T; getMapped: () => T };
class MappingLikeData<T> {
  getTarget: () => T;
  getMapped: () => T;
  constructor(data: T) {
...
> {
  ...
  constructor(options: {
    ...
    this._mapping =
      mapping && isNotEmpty(mapping) ? { ...mapping, ..._mapping } : null;
  }
  ...
  private async _getArray(
    ...
    try {
      ...
      const { data } = await this._client.$_call(this._apiId, {
      ...
      if (data[this._client.listKey]) {
        return data[this._client.listKey].map((item: any) =>
          this._mapping
            ...
            : new MappingLikeData(item as T)
        );
      } else {
        throw new Error(
          "This type of resource may not provided LIST method, or check your configurations."
        );
      }
    } catch (error) {
      // return rawList
  ...
  private async _get(
    ...
    try {
      ...
      return this._mapping
        ? (defineMapping(this._mapping, data) as MappingLike<T>)
        : (new MappingLikeData(data as T) as unknown as MappingLike<T>);
    } catch (error) {
