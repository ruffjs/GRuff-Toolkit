...
export default class RESTfulRequestor
  ...
  >(
    ...
    return this.$create_resource<T, FormData>(
      $(path),
      body,
      query,
      config
  ...
  >(
    ...
    return this.$get_resource<T, D>($([$(path), $(idOrkeys),
    $(subPath)]), query, config);
  }
  ...
  >(
    ...
    return this.$get_resource<T, D>($([$(path), $(idOrkeys),
    $(subPath), $(subIdOrkeys)]), query, config);
  }
