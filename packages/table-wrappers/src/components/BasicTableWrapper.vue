  ...
  <box width="100%" height="100%" background="--primary-bgcolor" :padding="padding">
    ...
    <box v-if="pagination" row="reverse" :height="paginationHeight" padding-top="13">
      <a-pagination size="small" :simple="simplePagination" :show-size-changer="(pagination as any).showSizeChanger"
        :show-quick-jumper="(pagination as any).showQuickJumper" showLessItems :value="(pagination as any).current"
        :current="(pagination as any).current" :total="(pagination as any).total"
        :page-size="(pagination as any).pageSize" :show-total="(total: number) => `共计 ${(pagination as any).total} 条`"
...
import { basic } from "../consts/props";
