...
export function pick<T = any>(this: RandomMethods, arr: T[], minLength: nummeric, maxLength: nummeric) {
    // pick( item1, item2 ... )
    if (!isArray(arr)) {
    ...
    if (minLength === 1 && maxLength === 1) return arr[this.natural(0, arr.length - 1)];

    return this.shuffle(arr, minLength, maxLength);
}

export function shuffle<T = any>(this: RandomMethods, arr: T[] = [], min?: nummeric, max?: nummeric) {
